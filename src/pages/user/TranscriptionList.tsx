...
import { getUserTranscriptions, deleteTranscription } from "../../services/transcribeApi";
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setRecentTranscriptions((prev) =>
        prev.filter((transcription) => transcription._id !== selectedTranscriptionId)
      );
      closeDeleteModal();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white dark:bg-gray-800 shadow-lg rounded-xl border border-gray-200 dark:border-gray-700">
          <div className="px-6 py-6">
            ...
            <div
              className={`${
                recentTranscriptions.length > 10 ? "max-h-96 overflow-y-auto" : ""
              }`}
            >
                ...
                <Link to={`/dashboard/transcription/${item._id}`} key={item._id}>
                  <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors my-4">
                    <div className="flex items-center space-x-4">
                      <div className={`p-2 rounded-lg ${item.type === "youtube" ? "bg-red-100 dark:bg-red-900/20" : item.type === "upload" ? "bg-purple-100 dark:bg-purple-900/20" : "bg-blue-100 dark:bg-blue-900/20"}`}>
                        {item.type === "youtube" ? (
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">Duration: {item.duration}s • Accuracy: {item.accuracy}</p>
                      </div>
                    </div>
